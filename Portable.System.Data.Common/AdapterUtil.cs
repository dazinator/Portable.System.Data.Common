...
using Resx = Portable.System.Data.Common.Properties.Resources;
...
namespace System.Data.Common
{
    internal static class ADP
    {
        ...
        static internal void CheckArgumentLength(string value, string parameterName)
        {
            ...
            if (0 == value.Length)
            {
                throw Argument(SR.Format(Resx.ADP_EmptyString, parameterName));
            }
        }
        ...
        {
            return Argument(SR.Format(Resx.ADP_ConnectionStringSyntax, index));
        }
        static internal ArgumentException KeywordNotSupported(string keyword)
        {
            return Argument(SR.Format(Resx.ADP_KeywordNotSupported, keyword));
        }
        static internal ArgumentException InvalidKeyname(string parameterName)
        {
            return Argument(SR.Format(Resx.ADP_InvalidKey), parameterName);
        }
        static internal ArgumentException InvalidValue(string parameterName)
        {
            return Argument(SR.Format(Resx.ADP_InvalidValue), parameterName);
        }
        static internal ArgumentException ConvertFailed(Type fromType, Type toType, Exception innerException)
        {
            return ADP.Argument(SR.Format(Resx.SqlConvert_ConvertFailed, fromType.FullName, toType.FullName), innerException);
        }
        ...
        {
            return InvalidOperation(SR.Format(Resx.ADP_InternalProviderError, (int)internalError));
        }
