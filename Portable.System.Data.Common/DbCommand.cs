...
namespace System.Data.Common
{
        ...
        IDisposable
    {
        ...
        protected virtual Task<DbDataReader> ExecuteDbDataReaderAsync(CommandBehavior behavior, CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
            {
                var taskFactory = new TaskFactory<DbDataReader>();
                return taskFactory.FromCancellation<DbDataReader>(cancellationToken);
            }
            else
            {
                CancellationTokenRegistration registration = new CancellationTokenRegistration();
                ...
                {
                    var taskFactory = new TaskFactory<DbDataReader>();
                    return taskFactory.FromException<DbDataReader>(e);
                }
                finally
                {
                    registration.Dispose();
        ...
        public virtual Task<object> ExecuteScalarAsync(CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
            {
                ...
                return taskFactory.FromCancellation<object>(cancellationToken);
            }
            else
            {
                CancellationTokenRegistration registration = new CancellationTokenRegistration();
                ...
                {
                    var taskFactory = new TaskFactory<object>();
                    return taskFactory.FromException<object>(e);
                }
                finally
                {
                    registration.Dispose();
